...
#include <list>
#include <cmath>
DIJKSTRA::DIJKSTRA(shared_ptr<Content>&content):_content(content)
{
...
void DIJKSTRA::Solve(int arr[COL][ROW])
{
    for (int x = 0; x < COL; x++)
            for (int y = 0; y < ROW; y++)
			{
    ...
    auto _distance = [](Node* a, Node* b)
		...
		while (!listNotTestedNodes.empty() && nodeCurrent != endNode)
		{
			...
			listNotTestedNodes.sort([](const Node* lhs, const Node* rhs) { return lhs->fLocalGoal < rhs->fLocalGoal; });
			...
			for (auto nodeNeighbour : nodeCurrent->vecNeighbours)
			{
				...
				float fPossiblyLowerGoal = _distance(nodeCurrent, nodeNeighbour);
